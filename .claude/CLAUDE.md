...
- **리뷰 필수 코드**: 보안, 성능, 정합성 측면에서 사용자가 확인해야 할 코드를 `파일명:라인번호` 형식으로 명시
- 테스트 현황과 알려진 제한사항

**리뷰 필수 코드 판단 기준:**
- 보안: 사용자 입력 처리, 인증/인가, 민감 데이터가 관련된 코드
- 성능: 동시성 처리, 외부 시스템 호출, 반복문 내 I/O가 있는 코드
...
**필요시 추가:**
- 상태 다이어그램: 상태 전이가 있는 경우
- 플로우차트: 복잡한 분기 로직이 있는 경우
